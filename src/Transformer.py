...
# 前節のDataLoaderなどを取得
from utils.dataloader import get_IMDb_DataLoaders_and_TEXT # DataLoader.pyのClass
...
" メモ "
# ある単語ID(行ID)に応じてその行(その単語の分散ベクトル表現)を返す
# ちなみに、TEXT.vocab.vectorsはボキャブラリー総単語数×分散表現の次元数(99万？×300？...単語数(行)×ベクトル(列))
# I    : [0.9, 0.7, ..., 0.1]
# like : [0.8, 0.2, ..., 0.2]
# cat  : [0.2, 0.5, ..., 0.8]
# ...
...
" メモ "
# 入力=24(ミニバッチ数)×256(一文の単語数)が、
# 出力=24×256×300(ベクトル)に変換される
# 単語1: [0.9, 0.7, ..., 0.1](=300)
# 単語2: ...
# ...
# 単語256(一文の最大): ...
...
class PositionalEncoder(nn.Module):
    ...
    " メモ "
    # これが無いとAttenntionを使った際に順番に関係なく、単語の関係の深さを決めてしまう可能性があるから語順などの位置情報を追加する
    # 足し算するだけ
...
" メモ "
# Embedderに位置情報を足し算しているだけなので、サイズは変わらない
...
"****************************"
# ここまでで単語列がベクトル表現になり、語順情報も付加される
# あとはこの特徴量を最終的にクラス分類でうまく処理できる特徴量へと変換するだけ
"****************************"
...
"****************************"
# TransformerBlockで特徴量変換している
# 以下のAttentionとFeedForwardはTransformerBlockで使われる
"****************************"
...
"****************************"
# 以下のTransformerBlockで上記の二つの関数を活用して特徴量変換している
"****************************"
